...
class TestSimple():
    def test_simple_add(self):
        ...
        inputs = {'A': 100, 'B': 12.9}
        assert hg.DFS(inputs, 'C') == 112.9, "Sum should be 112.9"

    def test_simple_multiply(self):
        hg = Hypergraph()
        hg.addEdge(['A', 'B'], 'C', R.Rmuliply)
        inputs = {'A': 3, 'B': 1.5}
        assert hg.DFS(inputs, 'C') == 4.5, "Product should be 4.5"

    def test_simple_subtract(self):
        hg = Hypergraph()
        hg.addEdge(['A', 'B'], 'C', R.Rsubtract)
        inputs = {'A': 3, 'B': 2}
        assert hg.DFS(inputs, 'C') == 1, "Product should be 4.5"
    ...
    def test_simple_void(self):
        ...
        inputs = {'A': 100, 'B': 51}
        assert hg.DFS(inputs, 'C') == None, "Should have invalid condition"
